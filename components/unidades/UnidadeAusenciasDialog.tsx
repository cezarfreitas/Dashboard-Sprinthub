...
import type { Unidade, Vendedor } from '@/hooks/unidades/useUnidades'
...
interface UnidadeAusenciasDialogProps {
  unidade: Unidade | null
  open: boolean
  onOpenChange: (open: boolean) => void
}

export const UnidadeAusenciasDialog = memo(function UnidadeAusenciasDialog({
  unidade,
  open,
  onOpenChange
}: UnidadeAusenciasDialogProps) {
  ...
  const carregarDados = useCallback(async () => {
    ...
    try {
      // Carregar vendedores
      const vendedoresRes = await fetch(`/api/unidades/${unidade.id}/vendedores`)
      if (vendedoresRes.ok) {
      ...
      // Carregar ausências
      const ausenciasRes = await fetch(`/api/fila/${unidade.id}/ausencias`)
      if (ausenciasRes.ok) {
  ...
  }, [unidade, open, carregarDados])

  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    e.preventDefault()
    
    if (!unidade || !selectedVendedor || !dataInicio || !dataFim || !motivo.trim() || !horaInicio || !horaFim) {
      return
    }
    ...
    try {
      setSaving(true)
      const response = await fetch(`/api/fila/${unidade.id}/ausencias`, {
        method: 'POST',
        headers: {
  ...
  }, [unidade, selectedVendedor, dataInicio, horaInicio, dataFim, horaFim, motivo, carregarDados])

  const handleDelete = useCallback(async (ausenciaId: number) => {
    if (!unidade) return

    try {
      const response = await fetch(`/api/fila/${unidade.id}/ausencias/${ausenciaId}`, {
        method: 'DELETE'
      })
      ...
      <DialogContent className="max-w-[95vw] sm:max-w-4xl max-h-[90vh] sm:max-h-[80vh] overflow-hidden flex flex-col p-4 sm:p-6">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-base sm:text-lg">
            ...
            <span className="truncate">Ausências - {unidade.name}</span>
          </DialogTitle>
