...
export default function PainelFiltersInline({
  ...
  return (
    <div className="flex flex-wrap items-center gap-4 mb-6 bg-gray-50 border border-gray-200 rounded-lg p-4 shadow-sm">
      ...
      <div className="flex flex-col gap-2 flex-1 min-w-[180px]">
        ...
        >
          <SelectTrigger className="h-9 text-xs bg-white border-gray-300 text-gray-900">
            <SelectValue placeholder="Selecione o período" />
          </SelectTrigger>
          <SelectContent className="bg-white border-gray-200 text-gray-900">
            {periodoOptions.map((option) => (
              <SelectItem key={option.value} value={option.value} className="text-xs text-gray-900 focus:bg-gray-100 focus:text-gray-900">
                {option.label}
              </SelectItem>
            ))}
          </SelectContent>
        ...
        <>
          <div className="flex flex-col gap-2 min-w-[150px]">
            ...
            <Input
              ...
              className="h-9 text-xs bg-white border-gray-300 text-gray-900"
            />
          </div>
          <div className="flex flex-col gap-2 min-w-[150px]">
      ...
      {/* Filtro de Funil */}
      <div className="flex flex-col gap-2 flex-1 min-w-[180px]">
        <Label className="text-xs font-medium text-gray-600">Funil</Label>
        <Select
        ...
        >
          ...
          <SelectContent className="bg-white border-gray-200 text-gray-900">
            <SelectItem value="todos" className="text-xs text-gray-900 focus:bg-gray-100 focus:text-gray-900">Todos</SelectItem>
            {funis.map((funil) => (
              <SelectItem key={funil.id} value={String(funil.id)} className="text-xs text-gray-900 focus:bg-gray-100 focus:text-gray-900">
                {funil.funil_nome}
              </SelectItem>
            ))}
          </SelectContent>
      ...
      <div className="flex flex-col gap-2 flex-1 min-w-[180px]">
        ...
        >
          ...
          <SelectContent className="bg-white border-gray-200 text-gray-900">
            <SelectItem value="todos" className="text-xs text-gray-900 focus:bg-gray-100 focus:text-gray-900">Todos</SelectItem>
            {grupos.map((grupo) => (
              <SelectItem key={grupo.id} value={String(grupo.id)} className="text-xs text-gray-900 focus:bg-gray-100 focus:text-gray-900">
                {grupo.nome}
              </SelectItem>
            ))}
          </SelectContent>
      ...
      <div className="flex flex-col gap-2 min-w-[280px]">
        <div className="flex items-center gap-2">
          ...
          <Label className="text-xs font-medium text-gray-600">Data de Ganho</Label>
        ...
        <div className="flex items-center gap-2">
          <Input
            ...
            placeholder="De"
            className="h-9 text-xs bg-white border-gray-300 text-gray-900 flex-1"
          />
      ...
      {filtrosAtivos && (
        <div className="flex flex-col gap-2 min-w-[100px]">
          <Label className="text-xs font-medium text-gray-600 opacity-0">Ações</Label>
          <Button
            variant="destructive"
            size="sm"
            onClick={() => setFiltros({
