...
export async function GET(request: NextRequest) {
  try {
    ...
    // 3. Criar tabela de relacionamento vendedores-unidades
    await executeQuery(`
    ...
    `)
    console.log('✅ Tabela vendedores_unidades criada/verificada')
    ...
    if (vendedoresCount[0].count === 0) {
      await executeQuery(`
        INSERT INTO vendedores (id, name, lastName, username, email, telephone) VALUES
    ...
    const relacionamentosCount = await executeQuery('SELECT COUNT(*) as count FROM vendedores_unidades') as any[]
    if (relacionamentosCount[0].count === 0) {
      await executeQuery(`
        INSERT INTO vendedores_unidades (vendedor_id, unidade_id) VALUES
        (1, 1), (2, 1), (3, 2), (4, 3), (5, 4)
      `)
      console.log('✅ Relacionamentos vendedores-unidades criados')
    }
    ...
    const stats = {
      ...
      relacionamentos: (await executeQuery('SELECT COUNT(*) as count FROM vendedores_unidades') as any[])[0].count,
